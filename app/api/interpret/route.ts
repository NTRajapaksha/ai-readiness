...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      teamScores,
      qualitativeWishes,
      selectedProvider, // optional override from UI: 'openai' | 'anthropic' | 'gemini' | 'groq'
    } = body;
...
IMPORTANT: Do NOT use markdown headers (like ###) or title headers. Output plain text starting directly with "Strategic Diagnosis:" followed by your paragraph, and then "Qualitative Synthesis:" followed by your paragraph.
    ...
    // 2. Google Gemini Support (Using gemini-flash-latest)
    ...
    // 5. Custom OpenAI-compatible / Local Ollama / OpenRouter
    if (customKey) {
    ...
    // 6. Smart Fallback Engine (Runs reliably when no external LLM key is configured)
