...
- **How it's implemented**: `generateRecommendations()` in `lib/recommendations.ts` evaluates `overallDims` and `teamDims`. Org-wide dimension scores below 50/100 trigger business-wide recommendations (`rec-fluency-org`, `rec-culture-org`, `rec-risk-org`, `rec-integration-org`, `rec-leadership-org`). Team dimension scores lagging the company average by $\ge 20$ points trigger targeted department recommendations (`rec-[team]-[dim]`). To ensure complete specificity without generic filler, `getTeamPlaybook(dim, teamName)` constructs one of **5 distinct dimension-specific 3-week playbook templates** (`fluency` $\rightarrow$ tool access & prompt mechanics; `integration` $\rightarrow$ task audit & workflow templates; `culture` $\rightarrow$ internal repos & peer standups; `risk` $\rightarrow$ data boundaries & PII masking; `leadership` $\rightarrow$ sponsor alignment & budget allocation). If all scores are $\ge 50$, an advanced coaching fallback (`rec-advanced-coaching`) triggers. Recommendations are sorted by priority (`high` first). Clicking a card sets `activePlaybookRec`, opening `PlaybookDrawer.tsx` to render the 3-week sprint breakdown.
